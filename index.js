...
async function run() {
  ...
  app.get("/allListings", async (req, res) => {
    ...
    let cursor = ListingCollection.find();
  ...
  app.get('/topRatedListings', async (req, res) => {
  try {
    const result = await ListingCollection.find()
      .sort({ likes: -1 })
      .limit(4)
      .toArray();
    res.send(result);
  } catch (error) {
    res.status(500).send({ message: 'Failed to fetch top rated listings', error });
  }
});
  try {
